...
// Lista de archivos BMP a procesar
...
int main() {
    omp_set_num_threads(800); // Ajusta al número real de núcleos!

    // Rutas de carpetas para entrada/salida
    ...
    for (int i = 0; i < 30; i++) {
        ...
        // Combina carpetas y nombres, así puedes tener muchas imágenes en subcarpetas
        ...
        bmp_invertirV(input_path, output_invV);        // Voltea verticalmente
        bmp_invertirH(input_path, output_invH);        // Espejo horizontal
        bmp_blur(input_path, output_blur, 57);         // Desenfoque (kernel 57)
    }
